...
def _service_hash(service_spec:str,criteria)->str:
	...
	for criterion in criteria:
		preimage+=_str_bytes(criterion.criterion_id) + _str_bytes(criterion.criterion_text)
...
def _policy_hash(max_evidence_age,required_corroboration_count,repair_allowed_field_mask,replay_scope,bindings)->str:
	...
	for binding in bindings:
		preimage+=_str_bytes(binding.authority_id) + _u32_bytes(binding.authority_revision) + _str_bytes(binding.role) + _str_bytes(binding.identity_kind) + _str_bytes(binding.identity_value) + _str_bytes(binding.canonical_origin)
	return _sha256_hex(preimage)
...
def _active_evidence_hash(covenant_id,delivery_hash:str,evidence_policy_hash:str,records)->str:
	...
	for record in records:
		...
		preimage+=_u64_bytes(record.covenant_id) + _u32_bytes(record.generation) + _str_bytes(record.evidence_id) + _str_bytes(record.authority_id) + _u32_bytes(record.authority_revision) + _str_bytes(record.subject) + _str_bytes(record.kind) + _str_bytes(record.source_kind) + _str_bytes(record.canonical_source) + _str_bytes(record.immutable_version_or_record_id) + _u64_bytes(record.published_at) + _u64_bytes(record.observed_at) + _u64_bytes(record.expires_at) + _hex32(record.content_digest) + (b'\x01' if bool(record.is_primary) else b'\x00') + _str_bytes(record.replaces_evidence_id)
	return _sha256_hex(preimage)
...
class Accord402(gl.Contract):
	...
	def _k(self,key:str,cov:CovenantRecord,obj,now:int,review_generation:int)->None:
		...
		if a==N:
			...
			for c in g[cb]:
				self.repair_authorizations_by_covenant[f].append(RepairAuthorizationRecord(u64(int(d.covenant_id)),u32(e),c[b8],u32(c[cq])))
			d.state=R
	...
	def get_accounting_totals(self)->AccountingTotalsView:
		return AccountingTotalsView(self.total_funded,self.total_closed_to_provider,self.total_closed_to_buyer,self.total_outstanding)
