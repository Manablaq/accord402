...
class _Recipient:
	...
	class Write:
		pass
def _fail(code:str)->NoReturn:raise gl.vm.UserError(code)
...
def bL(s:str)->None:
	...
	try:
		...
		_fail(ck)
	if len(raw)!=64 or not _is_lower_hex64(s):_fail(ck)
def _int_u32(x)->int:
	v=int(x)
	if v<0 or v>MAX_U32:_fail('U32_RANGE')
	return v
def _int_u64(x)->int:
	v=int(x)
	if v<0 or v>MAX_U64:_fail('U64_RANGE')
	return v
def by(x)->int:
	v=int(x)
	if v<0 or v>MAX_U256:_fail('U256_RANGE')
	return v
def Q(a,b,code:str)->int:
	ai=_int_u64(a)
	bi=_int_u64(b)
	if ai>MAX_U64-bi:_fail(code)
	return ai+bi
def am(a,b,code:str)->int:
	ai=by(a)
	bi=by(b)
	if ai>MAX_U256-bi:_fail(code)
	return ai+bi
def P()->int:
	...
	try:
		...
		_fail(cC)
	if v<0 or v>MAX_U64:_fail(cC)
	return v
def bO(covenant_id)->str:
	value=_int_u64(covenant_id)
	if value<=0:_fail('INVALID_COVENANT_ID')
	return str(value)
...
def _str_bytes(s:str)->bytes:
	raw=_utf8(s)
	if len(raw)>MAX_U32:_fail('STRING_TOO_LARGE')
	return len(raw).to_bytes(4,'big')+raw
def _hex32(s:str)->bytes:
	if not _is_lower_hex64(s):_fail('INVALID_DIGEST')
...
def bM(pairs):
	...
	return out
def aK(text:str):
	if K(text)>bj:raise ValueError('oversized json')
...
def bf(s:str)->None:
	...
	for b in raw:
		if b<=32 or b==127:_fail('SOURCE_WHITESPACE_OR_CONTROL')
def bA(label:str,final_label:bool)->bool:
	if len(label)<1 or len(label)>63:return False
	if label.startswith('xn--'):return False
	if final_label:
		if len(label)<2 or len(label)>63:return False
		for ch in label:
			if ch<'a' or ch>'z':return False
		return True
	first=label[0]
	last=label[-1]
	if not(first.isdigit()or 'a'<=first<='z'):return False
	if not(last.isdigit()or 'a'<=last<='z'):return False
	for ch in label:
		if not(ch.isdigit()or 'a'<=ch<='z' or ch=='-'):return False
	return True
def bk(host:str)->str:
	bf(host)
	if len(host)>253 or '.' not in host or host.startswith('.')or host.endswith('.'):_fail(cm)
	labels=host.split('.')
	if len(labels)<2:_fail(cm)
	denied_tlds={'local','localhost','internal','home','lan','test','invalid','onion'}
	for i,label in enumerate(labels):
		if not bA(label,i==len(labels)-1):_fail(cm)
	if labels[-1]in denied_tlds:_fail(cm)
	return host
def bE(origin:str)->str:
	L(origin,aG,cy,True)
	bf(origin)
	prefix=cJ
	if not origin.startswith(prefix):_fail(ci)
	rest=origin[len(prefix):]
	if cK in rest or '?' in rest or '#' in rest or('@' in rest)or(':' in rest):_fail(ci)
	host=bk(rest)
	if origin!=prefix+host:_fail(ci)
	return host
def aL(segment:str)->bool:
	if segment=='' or segment=='.' or segment=='..':return False
	for ch in segment:
		if not(ch.isascii()and(ch.isalnum()or ch in '._~-')):return False
	return True
def bF(source:str)->str:
	L(source,aH,cz,True)
	bf(source)
	prefix=cJ
	if not source.startswith(prefix):_fail(b3)
	...
	path=rest[slash:]
	bk(host)
	if '?' in path or '#' in path or '\\' in path or('%' in path):_fail(b3)
	if path==cK:return prefix+host
	if not path.startswith(cK)or path.endswith(cK)or '//' in path:_fail(b3)
	for segment in path[1:].split(cK):
		if not aL(segment):_fail(b3)
	return prefix+host
def af(source:str,version_id:str)->bool:
	...
	try:
		...
		return False
	if not aL(version_id):return False
	prefix=cJ
	if not source.startswith(prefix):return False
	rest=source[len(prefix):]
	slash=rest.find(cK)
	if slash<0:return False
	path=rest[slash:]
	if path==cK or not path.startswith(cK):return False
	return version_id in path[1:].split(cK)
def aC(source:str,version_id:str)->None:
	L(version_id,aA,cH,True)
	if not af(source,version_id):_fail('VERSION_ID_SOURCE_MISMATCH')
...
def _service_hash(service_spec:str,criteria)->str:
	...
	for criterion in criteria:preimage+=_criterion_bytes(criterion)
...
def _policy_hash(max_evidence_age,required_corroboration_count,repair_allowed_field_mask,replay_scope,bindings)->str:
	...
	for binding in bindings:preimage+=_authority_bytes(binding)
	return _sha256_hex(preimage)
def _delivery_hash(covenant_id,provider:Address,service_spec_hash:str,delivered_at,delivery_payload:str)->str:
	...
	if len(raw_address)!=20:_fail('INVALID_PROVIDER_ADDRESS')
...
def _active_evidence_hash(covenant_id,delivery_hash:str,evidence_policy_hash:str,records)->str:
	...
	for record in records:
		if int(record.covenant_id)!=int(covenant_id):_fail('FOREIGN_EVIDENCE_RECORD')
		preimage+=_evidence_bytes(record)
	return _sha256_hex(preimage)
def aU(covenant_key:str,evidence_id:str)->str:return 'I|'+covenant_key+'|'+evidence_id
def an(replay_scope:str,covenant_key:str,authority_id:str,evidence_id:str)->str:
	...
	if replay_scope==bo:return 'G|'+authority_id+'|'+evidence_id
	_fail(cG)
	return ''
def bx(binding)->tuple[str,str]:return(binding.identity_kind,binding.identity_value)
def az(bindings,authority_id:str,authority_revision)->AuthorityBinding:
	...
	for binding in bindings:
		if binding.authority_id==authority_id and int(binding.authority_revision)==revision:
			found=binding
			count+=1
	if count!=1 or found is None:_fail('AUTHORITY_BINDING_NOT_UNIQUE')
	return found
def ao(cov,bindings,item,now:int)->AuthorityBinding:
	...
	if item.source_kind not in(X,Y,bQ):_fail('INVALID_SOURCE_KIND')
	if item.source_kind in(X,Y)and item.immutable_version_or_record_id=='':_fail('MISSING_VERSION_ID')
	published=_int_u64(item.published_at)
	observed=_int_u64(item.observed_at)
	expires=_int_u64(item.expires_at)
	if published>observed or observed>now:_fail('INVALID_EVIDENCE_TIME_ORDER')
	if now-observed>int(cov.max_evidence_age):_fail('EVIDENCE_STALE_AT_INSERT')
	if expires<=now:_fail('EVIDENCE_EXPIRED_AT_INSERT')
	binding=az(bindings,item.authority_id,item.authority_revision)
	if binding.role==av:
		if not bool(item.is_primary):_fail(cD)
	elif binding.role==al:
		if bool(item.is_primary):_fail(cD)
	else:
		_fail(cE)
	origin=bF(item.canonical_source)
	if origin!=binding.canonical_origin:_fail('SOURCE_ORIGIN_MISMATCH')
	if item.source_kind in(X,Y):aC(item.canonical_source,item.immutable_version_or_record_id)
	return binding
def aM(records,bindings,required:int)->bool:
	primary_keys=[]
	corroborator_keys=[]
	for record in records:
		if record.source_kind not in(X,Y):continue
		if not af(record.canonical_source,record.immutable_version_or_record_id):continue
		binding=az(bindings,record.authority_id,record.authority_revision)
		key=bx(binding)
		if binding.role==av and bool(record.is_primary):
			if key not in primary_keys:primary_keys.append(key)
	for record in records:
		if record.source_kind not in(X,Y):continue
		if not af(record.canonical_source,record.immutable_version_or_record_id):continue
		binding=az(bindings,record.authority_id,record.authority_revision)
		...
		if binding.role==al and(not bool(record.is_primary)):
			if key not in primary_keys and key not in corroborator_keys:corroborator_keys.append(key)
	return len(primary_keys)>=1 and len(corroborator_keys)>=required
def T(value)->u64:
	current=int(value)
	if current>=aQ:return u64(aQ)
...
class Accord402(gl.Contract):
	...
	def _require_covenant(self,covenant_id:u64)->tuple[str,CovenantRecord]:
		c=covenant_id
		b=bO(c)
		if b not in self.covenants:_fail('COVENANT_NOT_FOUND')
		a=gl.storage.copy_to_memory(self.covenants[b])
		if int(a.covenant_id)!=int(c)or int(a.covenant_id)==0:_fail('COVENANT_ID_MISMATCH')
		return(b,a)
	def _o(self,key:str)->list[CriterionRecord]:
		b=[]
		for a in self.criteria_by_covenant[key]:b.append(gl.storage.copy_to_memory(a))
		return b
	def _n(self,key:str)->list[AuthorityBinding]:
		b=[]
		for a in self.authority_bindings_by_covenant[key]:b.append(gl.storage.copy_to_memory(a))
		return b
	def _q(self,key:str)->list[EvidenceRecord]:
		b=[]
		for a in self.evidence_records_by_covenant[key]:b.append(gl.storage.copy_to_memory(a))
		return b
	...
	def _h(self,key:str,cov:CovenantRecord,closure_reason:str)->None:
		a=cov
		if a.settlement_direction!=a9:_fail(cs)
		a.state=ac
	...
	def _g(self,cov:CovenantRecord,key:str,record:EvidenceRecord)->None:
		...
		if self.evidence_replay_keys.get(a,False)or self.evidence_replay_keys.get(b,False):_fail(cQ)
	...
	def open_covenant(self,terms:OpenCovenantInput)->u64:
		...
		if J.replay_scope not in(ba,bo):_fail(cG)
		if len(J.criteria)<1 or len(J.criteria)>bP:_fail('INVALID_CRITERIA_COUNT')
		if len(J.authority_bindings)<1 or len(J.authority_bindings)>bd:_fail('INVALID_AUTHORITY_COUNT')
		...
		for a in J.criteria:
			...
			if a.criterion_id in m:_fail('DUPLICATE_CRITERION_ID')
		...
		for a in J.authority_bindings:
			...
			if G in B:_fail('DUPLICATE_AUTHORITY_BINDING')
			B.append(G)
		if x>bS:_fail('OPEN_DYNAMIC_BUDGET_EXCEEDED')
		for a in J.authority_bindings:
			if a.role not in(av,al):_fail(cE)
			if a.identity_kind!=bH:_fail('INVALID_IDENTITY_KIND')
			H=bE(a.canonical_origin)
			if a.identity_value!=H:_fail('DOMAIN_IDENTITY_MISMATCH')
			l=(a.identity_kind,a.identity_value)
			if a.role==av:
				if l not in b:b.append(l)
			elif l not in c:c.append(l)
			v.append(a)
		if len(b)<1:_fail('PRIMARY_AUTHORITY_REQUIRED')
		g=_int_u32(J.required_corroboration_count)
		if g>aT:_fail('INVALID_CORROBORATION_COUNT')
		k=[]
		for A in c:
			if A not in b and A not in k:k.append(A)
		if g>len(k):_fail('INSUFFICIENT_CORROBORATION_CAPACITY')
		j=_int_u32(J.max_review_generations)
		if j<1 or j>be:_fail('INVALID_MAX_REVIEW_GENERATIONS')
		u=_int_u32(J.repair_allowed_field_mask)
		if u&~ap:_fail('INVALID_REPAIR_MASK')
		y=_int_u64(J.max_evidence_age)
		if y<=0 or y>a3:_fail('INVALID_MAX_EVIDENCE_AGE')
		...
		if o<Q(E,aP,'ACCEPTANCE_DEADLINE_OVERFLOW'):_fail('ACCEPTANCE_DEADLINE_TOO_SOON')
		if o>Q(E,aD,'ACCEPTANCE_HORIZON_OVERFLOW'):_fail('ACCEPTANCE_DEADLINE_TOO_FAR')
		if p<=o or p>Q(E,aN,'DELIVERY_HORIZON_OVERFLOW'):_fail('INVALID_DELIVERY_DEADLINE')
		if d<aE or d>aF:_fail('INVALID_CHALLENGE_DURATION')
		if n<aX or n>aY:_fail('INVALID_REPAIR_WINDOW')
		if r<a1 or r>a2:_fail('INVALID_RETRY_WINDOW')
		q=Q(p,d,cx)
		q=Q(q,ax,cx)
		if w<=p or w<q:_fail('ABSOLUTE_DEADLINE_TOO_SOON')
		if w>Q(E,ar,'ABSOLUTE_HORIZON_OVERFLOW'):_fail('ABSOLUTE_DEADLINE_TOO_FAR')
		h=by(J.principal)
		if h<=0 or int(gl.message.value)!=h:_fail('INVALID_FUNDING_AMOUNT')
		z=am(self.total_funded,h,'TOTAL_FUNDED_OVERFLOW')
		t=am(self.total_outstanding,h,'TOTAL_OUTSTANDING_OVERFLOW')
		if int(self.covenant_count)>=MAX_U64:_fail('COVENANT_ID_OVERFLOW')
	...
	def expire_unaccepted(self,covenant_id:u64)->None:
		...
		if a.state!=a7:_fail(bX)
		if P()<=int(a.acceptance_deadline):_fail('ACCEPTANCE_NOT_EXPIRED')
		self._m(b,a,aS)
	@gl.public.write
	def submit_delivery(self,covenant_id:u64,delivery_payload:str,evidence:list[EvidenceInput])->None:
		...
		j=P()
		if b.state!=aj:_fail(bX)
		if gl.message.sender_address!=b.provider:_fail(cv)
		if j>int(b.delivery_deadline):_fail('DELIVERY_DEADLINE_PASSED')
		L(l,bn,'DELIVERY_PAYLOAD_TOO_LONG',True)
		if len(m)<1 or len(m)>bm:_fail('INVALID_EVIDENCE_COUNT')
		e=K(l)
		f=[]
		for a in m:
			...
			if a.evidence_id in f:_fail('DUPLICATE_EVIDENCE_ID')
			f.append(a.evidence_id)
		if e>bK:_fail('DELIVERY_DYNAMIC_BUDGET_EXCEEDED')
		g=self._n(i)
		records=[]
		for a in m:
			ao(b,g,a,j)
			self._a(b,i,a.evidence_id,a.authority_id)
			records.append(EvidenceRecord(u64(int(b.covenant_id)),u32(0),a.evidence_id,a.authority_id,u32(int(a.authority_revision)),a.subject,a.kind,a.source_kind,a.canonical_source,a.immutable_version_or_record_id,u64(int(a.published_at)),u64(int(a.observed_at)),u64(int(a.expires_at)),a.content_digest,bool(a.is_primary),''))
		if not aM(records,g,int(b.required_corroboration_count)):_fail('TERMINAL_CAPABLE_EVIDENCE_STRUCTURE_REQUIRED')
		c=Q(j,b.challenge_duration,'CHALLENGE_DEADLINE_OVERFLOW')
		if c>=int(b.absolute_dispute_deadline):_fail('CHALLENGE_WINDOW_EXCEEDS_ABSOLUTE_DEADLINE')
	...
	def expire_non_delivery(self,covenant_id:u64)->None:
		b,a=self._require_covenant(covenant_id)
		if a.state!=aj:_fail(bX)
		if P()<=int(a.delivery_deadline):_fail('DELIVERY_NOT_EXPIRED')
		self._m(b,a,ak)
	@gl.public.write
	def challenge_delivery(self,covenant_id:u64,challenge_claim:str,challenged_criterion_ids:list[str])->None:
		...
		i=P()
		if a.state!=aI:_fail(bX)
		if gl.message.sender_address!=a.buyer:_fail('BUYER_ONLY')
		if i>int(a.challenge_deadline):_fail('CHALLENGE_DEADLINE_PASSED')
		if int(a.review_generation)!=0:_fail('CHALLENGE_ALREADY_ALLOCATED')
		if len(j)<1 or len(j)>ah:_fail('INVALID_CHALLENGE_COUNT')
		...
		for d in j:
			...
			c+=K(d)
			if d in h:_fail('DUPLICATE_CHALLENGED_CRITERION')
			h.append(d)
		if c>bD:_fail('CHALLENGE_DYNAMIC_BUDGET_EXCEEDED')
		f=self._o(g)
		b=0
		for e in f:
			if b<len(j)and e.criterion_id==j[b]:b+=1
		if b!=len(j):_fail('CHALLENGE_NOT_ORDERED_SUBSEQUENCE')
	...
	def authorize_unchallenged_settlement(self,covenant_id:u64)->None:
		b,a=self._require_covenant(covenant_id)
		if a.state!=aI:_fail(bX)
		if P()<=int(a.challenge_deadline):_fail('CHALLENGE_NOT_EXPIRED')
		self._h(b,a,bl)
	...
	def _execute_review_consensus(self,key:str,cov:CovenantRecord,now:int,review_generation:int)->str:
		...
		def derive_wire()->str:
			...
			for b in snapshot[cZ]:
				...
				if 200<=A<=299:
					...
					if snapshot[bY]&aa:s|=aa
					else:
						a=True
				else:
					g=True
				if s!=0:u.append({b8:b[b8],cq:s})
			...
			def base_wire(decision:str,failed:list[str],classification:str,repair_entries):return{cl:bw,b4:snapshot[b4],b9:snapshot[b9],cf:snapshot[cf],b7:snapshot[b7],b2:snapshot[b2],bZ:snapshot[bZ],co:decision,b0:snapshot[b0],b1:failed,ca:classification,cb:repair_entries}
			...
			D=[]
			for t in q:
				if not isinstance(t,str)or t in D or t not in e:return d
				D.append(t)
			for t in e:
				if t in D:z.append(t)
			if z!=q:return d
		...
		if not isinstance(a,str):_fail('INVALID_CONSENSUS_RESULT')
		if a==cd:_fail('UNREPAIRABLE_EVIDENCE_DEFECT')
		return a
	def _validate_consensus_wire(self,key:str,cov:CovenantRecord,wire_text:str,review_generation:int):
		...
		if (set(b.keys())!=bT) or (_canonical_json(b)!=r) or (type(b[cl])is not int or b[cl]!=bw) or (type(b[b4])is not int or b[b4]!=int(q.covenant_id)or b[b4]<=0 or(b[b4]>MAX_U64)) or (type(b[bZ])is not int or b[bZ]!=review_generation or b[bZ]<=0 or(b[bZ]>MAX_U32)):_fail(bW)
		for o,m in((b9,q.service_spec_hash),(cf,q.delivery_hash),(b7,q.evidence_policy_hash),(b2,q.active_evidence_set_hash)):
			if not isinstance(b[o],str)or not _is_lower_hex64(b[o])or b[o]!=m:_fail(bW)
		challenged=[value for value in self.challenged_criterion_ids_by_covenant[key]]
		if (b[b0]!=challenged or len(challenged)<1 or len(challenged)>ah) or (len(set(challenged))!=len(challenged)):_fail(bW)
		failed=b[b1]
		if (not isinstance(failed,list)or len(failed)>ah) or (any((not isinstance(x,str)for x in failed))or len(set(failed))!=len(failed)):_fail(bW)
		n=[x for x in challenged if x in failed]
		if n!=failed:_fail(bW)
		f=b[co]
		if f not in(S,U,M,N,Z):_fail(bW)
		a=b[ca]
		if a not in(au,V,W,O):_fail(bW)
		d=b[cb]
		if not isinstance(d,list)or len(d)>aw:_fail(bW)
		g=self._j(key)
		h=[]
		j=-1
		for i in d:
			if not isinstance(i,dict)or set(i.keys())!={b8,cq}:_fail(bW)
			l=i[b8]
			k=i[cq]
			if (not isinstance(l,str)or type(k)is not int) or (l in h or l not in g) or (k<1 or k>ap or k&~int(q.repair_allowed_field_mask)):_fail(bW)
			p=g.index(l)
			if p<=j:_fail(bW)
			j=p
			h.append(l)
		if f==S:
			if failed!=[]or a!=au or d!=[]:_fail(bW)
			e=[c.criterion_id for c in self._o(key)]
			if challenged!=e:_fail(bW)
		elif f==U:
			if len(failed)==0 or a!=V or d!=[]:_fail(bW)
		elif f==M:
			if failed!=[]or a!=au or d!=[]:_fail(bW)
			e=[c.criterion_id for c in self._o(key)]
			if challenged==e:_fail(bW)
		elif f==N:
			if failed!=[]or a!=W or len(d)==0:_fail(bW)
		elif failed!=[]or a!=O or d!=[]:_fail(bW)
		return b
	def _k(self,key:str,cov:CovenantRecord,obj,now:int,review_generation:int)->None:
		d=cov;e=review_generation;f=key;g=obj;d.review_generation=u32(e);d.adjudication_decision=g[co];d.failure_classification=g[ca];self._i(self.failed_criterion_ids_by_covenant,f,g[b1]);a=g[co]
		if a==S:
			self._h(f,d,aZ)
			return
		if a==M:
		...
		if a==U:
			...
			return
		if a==N:
			if d.repair_authorization_active:_fail('REPAIR_AUTHORIZATION_ALREADY_ACTIVE')
			for c in g[cb]:self.repair_authorizations_by_covenant[f].append(RepairAuthorizationRecord(u64(int(d.covenant_id)),u32(e),c[b8],u32(c[cq])))
			d.state=R;d.repair_authorization_generation=u32(e);d.repair_authorization_active=True;b=Q(now,d.evidence_repair_window,'REPAIR_DEADLINE_OVERFLOW');d.repair_deadline=u64(min(b,int(d.absolute_dispute_deadline)));d.retry_deadline=u64(0);self.covenants[f]=d
			return
		d.state=ad
	...
	def adjudicate_challenge(self,covenant_id:u64)->None:
		...
		d=P()
		if b.state!=aq:_fail(bX)
		if d>int(b.absolute_dispute_deadline):_fail(cr)
		a=int(b.review_generation)
		if a<1 or a>int(b.max_review_generations):_fail('INVALID_REVIEW_GENERATION')
	...
	def submit_evidence_repair(self,covenant_id:u64,replacements:list[EvidenceReplacementInput])->None:
		...
		F=P()
		if d.state!=R:_fail(bX)
		if gl.message.sender_address!=d.provider:_fail(cv)
		if F>int(d.repair_deadline):_fail('REPAIR_DEADLINE_PASSED')
		j=int(d.review_generation)
		if j>=int(d.max_review_generations):_fail(cw)
		if not d.repair_authorization_active or int(d.repair_authorization_generation)!=j:_fail('NO_ACTIVE_REPAIR_AUTHORIZATION')
		if len(I)<1 or len(I)>aw:_fail('INVALID_REPLACEMENT_COUNT')
		u=0
		t=[]
		y=[]
		for a in I:
			L(a.replaces_evidence_id,a0,'REPLACES_ID_TOO_LONG',True)
			if a.replaces_evidence_id in t:_fail('DUPLICATE_REPLACEMENT_TARGET')
			if a.evidence_id in y:_fail('DUPLICATE_REPLACEMENT_EVIDENCE_ID')
		...
		if u>bN:_fail('REPAIR_DYNAMIC_BUDGET_EXCEEDED')
		p=[]
		for z in self.repair_authorizations_by_covenant[l]:
			if int(z.generation)==j:p.append(gl.storage.copy_to_memory(z))
		if len(p)!=len(I):_fail('INCOMPLETE_REPAIR_SET')
		for i in range(len(p)):
			if I[i].replaces_evidence_id!=p[i].evidence_id:_fail('REPAIR_ORDER_OR_TARGET_MISMATCH')
		w=self._n(l)
		s=self._f(l)
		v={}
		n={}
		...
		for i,a in enumerate(I):
			...
			if f is None:_fail('REPAIR_TARGET_NOT_ACTIVE')
			A=int(z.field_mask)
			if A<1 or A>ap or A&~int(d.repair_allowed_field_mask):_fail('INVALID_REPAIR_AUTHORIZATION_MASK')
			ao(d,w,a,F)
			self._a(d,l,a.evidence_id,a.authority_id)
			if a.subject!=f.subject or a.kind!=f.kind or a.source_kind!=f.source_kind or(bool(a.is_primary)!=bool(f.is_primary)):_fail('NONREPAIRABLE_FIELD_CHANGED')
			...
			for H,C,B in E:
				if not bh(A,H,C,B):_fail('REPAIR_MASK_FIELD_MISMATCH')
			c=EvidenceRecord(u64(int(d.covenant_id)),u32(k),a.evidence_id,a.authority_id,u32(int(a.authority_revision)),a.subject,a.kind,a.source_kind,a.canonical_source,a.immutable_version_or_record_id,u64(int(a.published_at)),u64(int(a.observed_at)),u64(int(a.expires_at)),a.content_digest,bool(a.is_primary),a.replaces_evidence_id)
			x=aU(l,c.evidence_id)
			D=an(d.replay_scope,l,c.authority_id,c.evidence_id)
			if x in e or D in g:_fail('DUPLICATE_PENDING_REPLAY_KEY')
			e.append(x)
			g.append(D)
			m.append(c)
		...
		if not aM(h,w,int(d.required_corroboration_count)):_fail('REPAIR_BREAKS_TERMINAL_EVIDENCE_STRUCTURE')
	...
	def expire_repair(self,covenant_id:u64)->None:
		b,a=self._require_covenant(covenant_id)
		if a.state!=R:_fail(bX)
		if int(a.review_generation)>=int(a.max_review_generations):_fail('USE_EXPIRE_REVIEW_FOR_EXHAUSTION')
		if P()<=int(a.repair_deadline):_fail('REPAIR_NOT_EXPIRED')
		a.repair_authorization_active=False
		self._m(b,a,bb)
	@gl.public.write
	def retry_review(self,covenant_id:u64)->None:
		...
		d=P()
		if b.state!=ad:_fail(bX)
		if d>int(b.retry_deadline):_fail('RETRY_DEADLINE_PASSED')
		if d>int(b.absolute_dispute_deadline):_fail(cr)
		c=int(b.review_generation)
		if c>=int(b.max_review_generations):_fail(cw)
		a=c+1
	...
	def claim_settlement(self,covenant_id:u64)->None:
		...
		if a<=0 or a!=int(b.funded_amount):_fail('INVALID_OUTSTANDING_AMOUNT')
		if b.settlement_message_scheduled:_fail('SETTLEMENT_ALREADY_SCHEDULED')
		if b.state==ac:
			if b.settlement_direction!=aJ:_fail(ct)
			e=b.provider
			b.state=bg
			...
			if b.settlement_direction!=a4:_fail(ct)
			e=b.buyer
			b.state=bs
			...
			_fail(bX)
			return
		if int(self.total_outstanding)<a:_fail('GLOBAL_OUTSTANDING_UNDERFLOW')
	...
	@gl.public.view
	def get_accounting_totals(self)->AccountingTotalsView:return AccountingTotalsView(self.total_funded,self.total_closed_to_provider,self.total_closed_to_buyer,self.total_outstanding)
